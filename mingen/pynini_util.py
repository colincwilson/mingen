...
# Create acceptors, unions, sigstar from symbol lists
# (cf. strings), compile rules from mingen format and
# apply to symbol lists.
...
def accep(x, symtable):
    """
    Map space-separated sequence of symbols to acceptor
    (identity transducer).
...
# todo: pynini built-in?
def union(fsts):
    """
    Union list of Fsts.
    """
    fst = pynini.union(*fsts)
    return fst  # todo: check symbol table
...
def compile_rule(A, B, C, D, sigstar, symtable):
    """
    Compile cdrewrite rule from A -> B / C __D where 
    A and B are space-separated strings,
    C and D are segment regexs (seg1|seg2|...).
    """
    # Use explicit epsilon for deletion rules,
    # instead of pynutil.delete(), and mark rewrite
    # loci (for checking whether rule applies to input).
    if B == '∅':
        B = config.epsilon
    B = ' '.join(['⟨', B, '⟩'])
    # Insertion rule.
...
def rewrite(rule, inpt, sigstar=None, symtable=None):
    """
    Determine whether inpt is within scope of rule and 
    if so return result of applying the rule.
    """
    ...
    strpath_iter = pred_fst.paths(input_token_type=symtable,
                                  output_token_type=symtable)
    pred = [x for x in strpath_iter.ostrings()][0]  # xxx
    if re.search('⟨', pred):  # In-scope.
