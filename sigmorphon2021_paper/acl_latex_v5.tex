...
Our method can be applied to any irregular (or indeed regular) change. For \textipa{i} $\to$ \textipa{Ept} (as in \emph{sleep} $\sim$ \emph{slept}), the close neighbors include \textipa{/gip/} ($.85$) and \textipa{/flip/} ($.73$, one of Albright \& Hayes's wug stems) while among the near misses are \textipa{/fip/}, \textipa{/vip/}, \textipa{/nip/}, and \textipa{/snip/}. Would the nonce past form \textipa{/gEpt/} be far more acceptible than \textipa{/fEpt/}? We look forward to future empirical tests of the minimal generalization model, along these lines and others, as part of the collective effort to determine where we are and how much further we have to go in cognitive modeling of inflection.
